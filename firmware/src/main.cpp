...
void print_telemetry() {
  ...
  Serial.printf("},\"time\":{\"rtc\":{\"year\":%u,\"day_of_year\":%u,\"hour\":%u,\"status\":\"%s\"},\"gps\":{\"year\":%u,\"day_of_year\":%u,\"hour\":%u,\"status\":\"%s\"}},\"model\":{\"status\":\"%s\",\"checksum\":\"%s\",\"features_ready\":%s", snapshot.time.year, snapshot.time.day_of_year, snapshot.time.hour, indra::status_name(snapshot.time.status), snapshot.gps_time.year, snapshot.gps_time.day_of_year, snapshot.gps_time.hour, indra::status_name(snapshot.gps_time.status), indra::model_status_name(result.status), result.model_checksum, features_ready ? "true" : "false");
