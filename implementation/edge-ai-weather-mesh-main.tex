...
of approximately INR 19,130 (full hazard configuration) or INR 13,530
...
\midrule
\textbf{Total per Node} & \textbf{Complete research-grade node} & \textbf{19,130} \\
\bottomrule
\end{tabular}
\end{table}
...
At approximately INR 19,130 per node, a 20-node mesh network represents a
...
developed and is ready for hardware integration. The immediate next phase
involves capturing baseline tabular data from the 12-sensor array,
validating power consumption during localized inference loops, and
deploying the trained XGBoost model to validate hazard detection accuracy.
Once validated, the node is ready to replicate and scale across
mountainous topography, establishing the foundation of a true
swarm-intelligence environmental network.
